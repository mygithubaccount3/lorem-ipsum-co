        ...
        $.ajax("https://5cbef81d06a6810014c66193.mockapi.io/api/comments/" + sessionStorage.id,
            {
                ...
                "data": "{\n    \"created_at\": " + Date.now() + ",\n    \"title\": " + "\"" + $(e.target).nextAll('.comment-title').text() + "\"" + ",\n    \"body\": " + "\"" + $(e.target).nextAll('.comment-text').text() + "\"" + "\n}",
                "beforeSend": function () {
                    $(e.target).nextAll('.comment-title, .comment-text').removeAttr('contenteditable').css('box-shadow', 'none');
                    $('<div class="comments-loader" style="position: absolute; top: 0; right: 0; bottom: 0; left: 0; background-color: white; opacity: 0.5"><i class="fa fa-cog" aria-hidden="true"></i></div>').insertAfter('.comment-save')
                },
                "complete": function () {
